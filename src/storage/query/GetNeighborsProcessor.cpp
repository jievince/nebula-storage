...
ErrorCode GetNeighborsProcessor::checkAndBuildContexts(const cpp2::GetNeighborsRequest& req) {
...
ErrorCode GetNeighborsProcessor::buildTagContext(const cpp2::TraverseSpec& req) {
...
ErrorCode GetNeighborsProcessor::buildEdgeContext(const cpp2::TraverseSpec& req) {
...
ErrorCode GetNeighborsProcessor::handleEdgeStatProps(
    ...
    for (size_t statIdx = 0; statIdx < statProps.size(); statIdx++) {
        ...
        if (exp == nullptr) {
            return ErrorCode::E_INVALID_STAT_TYPE;
        }
...
ErrorCode GetNeighborsProcessor::checkStatType(const meta::SchemaProviderIf::Field* field,
