...
ErrorCode QueryBaseProcessor<REQ, RESP>::handleVertexProps(
    ...
    for (auto& vertexProp : vertexProps) {
        ...
        if (iter == tagContext_.schemas_.end()) {
            ...
            return ErrorCode::E_TAG_NOT_FOUND;
        }
...
template<typename REQ, typename RESP>
ErrorCode QueryBaseProcessor<REQ, RESP>::handleEdgeProps(
    ...
    for (auto& edgeProp : edgeProps) {
        ...
        if (iter == edgeContext_.schemas_.end()) {
            ...
            return ErrorCode::E_EDGE_NOT_FOUND;
        }
        CHECK(!iter->second.empty());
...
template<typename REQ, typename RESP>
ErrorCode QueryBaseProcessor<REQ, RESP>::buildYields(const REQ& req) {
...
template<typename REQ, typename RESP>
ErrorCode QueryBaseProcessor<REQ, RESP>::buildFilter(const REQ& req) {
    const auto& traverseSpec = req.get_traverse_spec();
    ...
    if (!filterStr.empty()) {
        ...
        if (filter_ == nullptr) {
            return ErrorCode::E_INVALID_FILTER;
        }
...
template <typename REQ, typename RESP>
ErrorCode QueryBaseProcessor<REQ, RESP>::getSpaceVertexSchema() {
...
template <typename REQ, typename RESP>
ErrorCode QueryBaseProcessor<REQ, RESP>::getSpaceEdgeSchema() {
...
template <typename REQ, typename RESP>
ErrorCode QueryBaseProcessor<REQ, RESP>::checkExp(const Expression* exp,
                                                        bool returned,
                                                        bool filtered,
                                                        bool updated) {
    switch (exp->kind()) {
        ...
        case Expression::Kind::kVar: {
            ...
            return varExp->isInner() ? ErrorCode::SUCCEEDED
