...
public:
    ...
    void process(const cpp2::TransLeaderReq& req) {
        ...
        if (!status.ok()) {
            LOG(ERROR) << "Space: " << spaceId << " Part: " << partId << " not found";
            this->pushResultCode(ErrorCode::E_PART_NOT_FOUND, partId);
            onFinished();
            return;
        }
        ...
        if (partMeta.hosts_.size() == 1) {
            ...
            this->pushResultCode(ErrorCode::E_TRANSFER_LEADER_FAILED, partId);
            onFinished();
            return;
        }
        ...
        part->asyncTransferLeader(host,
            ...
            if (code == kvstore::ResultCode::ERR_LEADER_CHANGED) {
                ...
                folly::via(folly::getIOExecutor().get(), [this, part, spaceId, partId] {
                    ...
                    this->pushResultCode(ErrorCode::E_RETRY_EXHAUSTED, partId);
                    onFinished();
                });
            } else {
                LOG(ERROR) << "Space " << spaceId << " part " << partId
...
public:
    ...
    void process(const cpp2::AddPartReq& req) {
        ...
        if (FLAGS_store_type != "nebula") {
            this->pushResultCode(ErrorCode::E_INVALID_STORE, partId);
            onFinished();
            return;
        }
